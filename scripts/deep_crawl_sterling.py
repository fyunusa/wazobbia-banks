"""Specialized Deep Crawler for Sterling Bank"""
...
logger = logging.getLogger("deep_crawl_sterling")

BANK_HOME = "https://www.sterlingbankng.com"
DATA_DIR = Path("data/sterling")
TIMEOUT = 60000

class SterlingDeepCrawler:
    def __init__(self):
        self.visited_urls: Set[str] = set()
        self.page_queue = [BANK_HOME]
        self.page_count = 0
        self.failed_urls = {"error": []}
    
    async def crawl(self):
        from playwright.async_api import async_playwright
        logger.info(f"🚀 Starting deep crawl of STERLING BANK")
        ...
        with open(DATA_DIR / "index.json", "w") as f:
            json.dump({"bank": "Sterling", "total_pages": self.page_count}, f)
...
async def main():
    await SterlingDeepCrawler().crawl()
