"""Specialized Deep Crawler for Ecobank Nigeria"""
...
logger = logging.getLogger("deep_crawl_ecobank")

BANK_HOME = "https://www.ecobank.com/en-ng/"
DATA_DIR = Path("data/ecobank")
FILES_DIR = DATA_DIR / "files"
TIMEOUT = 60000

FILES_DIR.mkdir(parents=True, exist_ok=True)

class EcobankDeepCrawler:
    def __init__(self):
    ...
    async def crawl(self):
        ...
        logger.info(f"🚀 Starting deep crawl of ECOBANK NIGERIA")
                ...
                try:
                    ...
                    for link in links:
                        try:
                            ...
                            if urlparse(abs_url).netloc == urlparse(BANK_HOME).netloc:
                                if self._is_file_url(abs_url):
                                    await self._download_file(abs_url)
                                elif abs_url not in self.visited_urls and abs_url not in self.page_queue:
        ...
        with open(DATA_DIR / "index.json", "w") as f:
            json.dump({"bank": "Ecobank", "total_pages": self.page_count}, f)
                ...
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                    if resp.status == 200:
                        ...
                        with open(filepath, "wb") as f:
                            f.write(content)
                        logger.info(f"   ⬇️  Downloaded: {filename} ({len(content)/1024:.1f}KB)")
                    else:
                        logger.error(f"   ✗ HTTP {resp.status}: {filename}")
        except Exception as e:
            logger.error(f"   ✗ Download failed: {filename} - {type(e).__name__}")

async def main():
    await EcobankDeepCrawler().crawl()
