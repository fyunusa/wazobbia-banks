"""Specialized Deep Crawler for Standard Chartered Nigeria"""
...
logger = logging.getLogger("deep_crawl_stanbic_ng")

BANK_HOME = "https://www.sc.com/ng/"
DATA_DIR = Path("data/standardchartered")
TIMEOUT = 60000

class StdCharteredDeepCrawler:
    def __init__(self):
    ...
    async def crawl(self):
        ...
        logger.info(f"🚀 Starting deep crawl of STANDARD CHARTERED NIGERIA")
        logger.info(f"🔗 Homepage: {BANK_HOME}")
        
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        
        # Load previously crawled URLs to resume from where we left off
        self._load_existing_visited_urls()        async with async_playwright() as p:
        ...
        with open(DATA_DIR / "index.json", "w") as f:
            json.dump({"bank": "Standard Chartered", "total_pages": self.page_count}, f)
...
async def main():
    await StdCharteredDeepCrawler().crawl()
