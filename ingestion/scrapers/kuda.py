...
logger = logging.getLogger("ingestion.scrapers.kuda")


class KudaScraper(BaseScraper):
    """Scraper implementation for Kuda Microfinance Bank.
    
    Supports two modes:
    1. JSON mode: Reads pre-collected data from data/kuda/ directory
    2. Live mode: Scrapes live from Kuda website
    ...
    async def scrape(self) -> List[RawDocument]:
        """Load Kuda data from pre-collected JSON files only.
        ...
        logger.error(f"   Please run: python scripts/collect_bank_data.py kuda")
        return []
